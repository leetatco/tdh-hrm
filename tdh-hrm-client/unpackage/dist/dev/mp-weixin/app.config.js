"use strict";
const common_function_myPubFunction = require("./common/function/myPubFunction.js");
const config = {
  ...
  checkSharePages: {
    ...
    // ['shareAppMessage', 'shareTimeline'], // menus内的数组可多选，shareAppMessage 转发给朋友，shareTimeline 分享到朋友圈，需要mode为1或2才会生效
    list: [
      "/pages/index/*",
      "/pages/goods/*",
      "/pages_template/*"
  ...
  checkEncryptRequest: {
    ...
    list: [
      ...
      // 表示 template/test/pub/testEncryptRequest 云函数需要加密通信
      "^template/encrypt/(.*)"
      // 表示以 template/encrypt/ 开头的云函数或云对象需要加密通信
    ]
  },
  // 静态文件的资源URL地址
  staticUrl: {
