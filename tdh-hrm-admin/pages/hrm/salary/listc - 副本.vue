...
<script>
	...
	export default {
		...
		methods: {
			...
			//返回休息文字描述
			toRestTypeText(item) {
				if (item.rest_type == 1) return '单休';
				if (item.rest_type == 2) return '双休';
				if (item.rest_type == 3) {
					return '大小周';
				} else {
					if (item.salary_type == 2) return `${item.total_salary}元/天`;
					if (item.salary_type == 3) return `${item.total_salary}元/时`;
				}
			},
			// 显示添加页面
			addBtn() {
