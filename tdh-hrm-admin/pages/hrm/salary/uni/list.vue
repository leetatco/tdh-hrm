<template>
	<view class="page-body">
		...
		<view>
			<el-row>
				<el-button type="success" size="small" icon="el-icon-circle-plus-outline" v-if="$hasRole('admin') || $hasPermission('hr-add')" @click="addBtn">添加</el-button>
				<!-- 批量操作 -->
	...
	export default {
		data() {
			...
			return {
				...
				table1: {
					// 表格数据请求地址
					action: "admin/hrm/salary/sys/uni/getList",
					//按钮显示
					rightBtns: [{
							mode: 'detail_auto',
							title: '详细',
							show: (item) => {
								return this.$hasRole('admin') || this.$hasPermission('hr-read')
							}
						},
						{
							mode: 'update',
							title: '编辑',
							show: (item) => {
								return this.$hasRole('admin') || this.$hasPermission('hr-update')
							}
						},
						{
							mode: 'delete',
							title: '删除',
							show: (item) => {
								return this.$hasRole('admin') || this.$hasPermission('hr-delete')
							}
						}
					],
					// 表格字段显示规则
					columns: [						
						{"key":"roles","title":"角色组","type":"text","width":200},
						{"key":"start_level","title":"职级起","type":"number","width":200},
						{"key":"end_level","title":"职级讫","type":"number","width":200},
						{"key":"update_date","title":"更新时间","type":"time","width":200,"show": ["detail"]},
						{"key":"users.nickname","title":"更新人","type":"text","width":200,"show": ["detail"]}
					],					
					// 多选框选中的值
					multipleSelection: [],
					// 当前高亮的记录
				...
				queryForm1: {
					...
					columns: [						
						{"key":"roles","title":"角色组","type":"text","width":200,"mode":"%%"},
						{"key":"start_level","title":"职级起","type":"number","width":200,"mode":"="},
						{"key":"end_level","title":"职级讫","type":"number","width":200,"mode":"="},
				...
				form1: {
					...
					props: {
						...
						columns: [		
							{
							  key: "roles", title: "角色组", type: "table-select", placeholder: "请选择角色",
		...
		methods: {
			...
			addBtn() {
				this.resetForm();
				this.form1.props.action = 'admin/hrm/salary/sys/uni/add';
			...
			// 显示修改页面
			updateBtn({ item }) {
				this.form1.props.action = 'admin/hrm/salary/sys/uni/update';
			...
			deleteBtn({ item, deleteFn }) {
				deleteFn({
					action: "admin/hrm/salary/sys/uni/delete",
					data: {
						_id: item._id
					},
				});
