<template>
	<view class="page-body">
		...
		<view class="vk-table-button-box">
			...
			<el-button type="danger" size="small" icon="el-icon-s-tools" :disabled="!table1.selectItem"
						...
						},
						{
							...
							formatter: (val, row, column, index) => {
								...
								if (row.role_id === "admin") {
									...
									val.map((item, index) => {
										if (vk.pubfn.isNotNull(item.company_id)) {
					...
					props: {
						...
						columns: [{
							...
							},							
							{
								...
								type: "tree-select",
								multiple: true,
								action: "admin/hrm/company/pub/getList",
								props: {
									list: "rows",
									value: "company_id",
									label: "company_name",									
