<template>
	<view class="header no-user-select" v-loading="!vk.getVuex('$app.inited')" :style="'--textColor'+textColor">
		...
		<view class="left">
			...
			<navigator class="logo-mode-2" open-type="reLaunch" url="/">
				<view class="logo-box" v-show="!vk.getVuex('$app.leftCollapse')">
					<image class="logo-image" :src="vk.getVuex('$app.staticUrl.navBar.logo')" mode="scaleToFill">
					</image>
					<view class="app-name">集团管理系统</view>
				</view>
				<view class="logo-box" v-show="vk.getVuex('$app.leftCollapse')">
				...
				<view class="navbar" :class="{ 'navbar-mini': !matchLeftWindow, 'popup-menu': popupMenuOpened }">
					...
					<view class="navbar-right pointer">
						...
						<view @click="togglePopupMenu" class="navbar-user">
							...
							<vk-data-icon class="arrowdown" name="vk-icon-unfold" :color="textColor"
								size="13"></vk-data-icon>
						</view>
						...
						<view class="navbar-menu">
							...
							<view v-if="vk.getVuex('$error.logs').length" @click="openForm('errorLog')"
								...
								<el-badge :value="vk.getVuex('$error.logs').length" class="item">
									<vk-data-icon name="el-icon-message-solid" size="22"
										:color="textColor"></vk-data-icon>
								</el-badge>
							</view>
							<!-- 通知图标 
							<view class="menu-item debug pointer" @click="openNotification">-->
							<view class="menu-item debug pointer">
								<el-badge :value="vk.getVuex('$user.unReadCount')" class="item">
									<vk-data-icon name="el-icon-bell" size="22" :color="textColor"></vk-data-icon>
								</el-badge>
							</view>
							<!-- 待办审核 
							<view class="menu-item debug pointer" @click="viewApplication">
								<el-badge :value="vk.getVuex('$user.unProcessCount')" class="item">
									<text>待办审核</text> </el-badge>
							</view>-->
							<!-- #endif -->
							...
							<view class="menu-item text-overflow">
								<text>{{ vk.getVuex("$user.userInfo.nickname") }}</text>
							</view>
		...
		<!-- 弹窗 - 通知列表 -->
		<notificationList v-model="formDatas.notificationList" @update-count="loadUnreadCount"></notificationList>
	</view>
</template>
...
<script>
	...
	import notificationList from "./components/bpmn-notification-list"; // 新增通知列表组件
	...
	export default {
		...
		methods: {
			...
			openForm(name) {
				...
				// 确保使用响应式的方式更新
				that
					.$set(that.formDatas, name, {
						show: true
					});
			},
			// 打开通知列表
			async openNotification() {
				await this.loadUnProcessCount()
				this.openForm('notificationList');
			},
			// pc状态下菜单折叠
			menuCollapse() {
				let leftCollapse = this.vk.getVuex('$app.leftCollapse');
				this.vk.setVuex('$app.leftCollapse', !leftCollapse);
				this.checkMenuCollapse();
			},
			checkMenuCollapse() {
				let leftCollapse = this.vk.getVuex('$app.leftCollapse');
