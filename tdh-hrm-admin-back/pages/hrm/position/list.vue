<template>
	<view class="page-body">
		...
		<vk-data-table ref="table1" :action="table1.action" :columns="table1.columns" :query-form-param="queryForm1"
			...
			<!-- 排序值 -->
			<template v-slot:job_level="{ row, column, index }"
				v-if="$hasRole('admin') || $hasPermission('hrm-employee-config-add')">
				<el-input-number v-model="row.job_level" size="mini" :precision="0" :min="1"
					@change="joblevelChange($event, row)" />
			</template>
		</vk-data-table>
		<!-- 表格组件结束 -->
...
<script>
	...
	export default {
		...
		methods: {
			...
			// 修改职级值
			joblevelChange(job_level, item) {
				vk.callFunction({
					url: 'admin/hrm/position/sys/update',
					data: {
						_id: item._id,
			...
			addBtn() {
				this.resetForm();
				this.form1.props.action = 'admin/hrm/position/sys/add';
			...
			updateBtn({
				...
				this.form1.props.action = 'admin/hrm/position/sys/update';
			...
			deleteBtn({
				...
				deleteFn({
					action: "admin/hrm/position/sys/delete",
					data: {
						_id: item._id
					},
				});
