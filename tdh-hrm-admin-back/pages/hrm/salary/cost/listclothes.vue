<template>
	<view class="page-body">
		...
		<!-- 添加或编辑的弹窗开始 -->
		<vk-data-dialog v-model="form1.props.show" :title="form1.props.title" width="2100rpx" mode="form"
	...
	export default {
		data() {
			...
			return {
				...
				table1: {
					// 表格数据请求地址
					action: "admin/hrm/salary/sys/clothes/getList",
					//按钮显示
					rightBtns: [{
							mode: 'detail_auto',
							title: '详细',
		...
		methods: {
			...
			addBtn() {
				this.resetForm();
				this.form1.props.action = 'admin/hrm/salary/sys/clothes/add';
			...
			updateBtn({
				...
				this.form1.props.action = 'admin/hrm/salary/sys/clothes/update';
			...
			deleteBtn({
				...
				deleteFn({
					action: "admin/hrm/salary/sys/clothes/delete",
					data: {
						_id: item._id
					},
				});
				...
				this.$refs.form1.validate((valid) => {
					if (valid) {
						...
						this.form1.data.employee_list.forEach(async (e) => {
							e.attendance_ym = this.form1.data.attendance_ym;
							let data = await vk.callFunction({
								url: 'admin/hrm/salary/sys/clothes/add',
								title: '请求中...',
								data: e
							})
							if (data.code !== 0) {
								return vk.alert(e, "新增不成功", "确定")
							}
						})
						vk.alert("新增成功", "提示", "确定", () => {
							// 点击确定按钮后的回调
			...
			getYears(val, formData, column, index, option) {
