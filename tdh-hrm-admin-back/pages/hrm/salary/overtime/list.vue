	...
	export default {
		data() {
			...
			return {
				...
				table1: {
					// 表格数据请求地址
					action: "admin/hrm/salary/sys/overtime/getList",
					//按钮显示
					rightBtns: [{
							mode: 'detail_auto',
							title: '详细',
		...
		methods: {
			...
			addBtn() {
				this.resetForm();
				this.form1.props.action = 'admin/hrm/salary/sys/overtime/add';
			...
			updateBtn({
				...
				this.form1.props.action = 'admin/hrm/salary/sys/overtime/update';
			...
			deleteBtn({
				...
				deleteFn({
					action: "admin/hrm/salary/sys/overtime/delete",
					data: {
						_id: item._id
					},
				});
