<template>
	<view class="page-body">
		...
		<view>
			<el-row>
				<el-button type="success" size="small" icon="el-icon-circle-plus-outline" v-if="$hasRole('admin') || this.$hasPermission('hrm-employee-config-add')" @click="addBtn">添加</el-button>
				<!-- 批量操作 -->				
			</el-row>
		</view>
		<!-- 自定义按钮区域结束 -->
...
<script>
	...
	export default {
		...
		methods: {
			...
			addBtn() {
				this.resetForm();
				this.form1.props.action = 'admin/hrm/resigntype/sys/add';
			...
			updateBtn({
				...
				this.form1.props.action = 'admin/hrm/resigntype/sys/update';
			...
			deleteBtn({
				...
				deleteFn({
					action: "admin/hrm/resigntype/sys/delete",
					data: {
						_id: item._id
					},
				});
